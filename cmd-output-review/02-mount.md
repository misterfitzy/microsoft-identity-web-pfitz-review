...
## Summary
Shows all mounted filesystems including:
- Root filesystem: `/dev/sda1` on `/` (ext4)
- Boot partition: `/dev/sda16` on `/boot` (ext4)
- EFI partition: `/dev/sda15` on `/boot/efi` (vfat)
- Additional storage: `/dev/sdb1` on `/mnt` (ext4)
- Various virtual filesystems (proc, sysfs, tmpfs, etc.)
